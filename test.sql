...
--
--sql hist ---
-- This is an update.,..This is an further Updatet
--
...
FROM
    v$sql v,
    dba_hist_active_sess_history u
WHERE
    TO_DATE(v.first_load_time,'YYYY-MM-DD hh24:mi:ss') > add_months(trunc(SYSDATE,'MM'),-2)
AND
    u.sql_id = v.sql_id
AND
    v.parsing_schema_name IN (
        'BI_SA','BI_DWH'
    )
AND
    u.machine <> 'ovs5s.sigma-it.local'
ORDER BY
    v.first_load_time DESC;
...
select temporary, last_analyzed, table_name from all_tables  order by LAST_ANALYZED desc nulls last;
...
SELECT DBMS_SQLTUNE.REPORT_SQL_MONITOR(sql_id => '9vs9xtp4ajznk', session_id =>21, type=>'ACTIVE') from dual;
